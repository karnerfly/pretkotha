...
const ContentCard = ({ type, title, description, image, views, likes, tag, featured, actionText, actionIcon }) => {
    return (
        <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 group">
            <div className="relative">
                ...
                {tag && <Badge text={tag} color={type === "drawing" ? "bg-secondary-500" : "bg-primary-500"} />}

                {/* Badge Component for Featured */}
                {featured && <Badge text="Featured" color="bg-yellow-500" position="left" />}
            </div>
            <div className="p-5">
                <div className="flex justify-between items-start mb-2">
                    <h3 className="text-xl font-semibold text-gray-800 group-hover:text-primary-600 transition-colors">{title}</h3>
